...
export default function HeartExplorer() {
  ...
  const heartHotspots = useMemo(
    () => Object.fromEntries(HEART_STRUCTURES.map((s) => [s.id, s.pos])),
    []
  );
        ...
        <div className="lg:col-span-2 relative h-[480px] lg:h-[620px] rounded-3xl glass overflow-hidden grid-overlay">
          ...
          {currentTour && (
            <TourPlayer
              ...
              onFocus={(id) => {
                setActive(id);
                const stepIdx = currentTour.steps.findIndex((s) => s.focus === id);
                if (stepIdx >= 0) setTourStep(stepIdx);
              }}
              hotspotPositions={heartHotspots}
            />
          )}
        </div>
